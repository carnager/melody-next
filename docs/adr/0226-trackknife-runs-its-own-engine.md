...
`$XDG_RUNTIME_DIR/tkengine.sock`. If nothing answers, it starts `tkengine`
detached, in its own session, with output going to `tkengine.log` in the data
...
The program is `$TRACKKNIFE_ENGINE`, else the `tkengine` beside the
executable (installed, or the build tree's `src/daemon`), else the one on
`PATH`.

## The database

`tkengine` opens one file, `lists.sqlite`, for the catalogue and the workspace
...
earlier `tkengine` builds are no longer read.
...
A workspace in Qt's test mode never starts an engine: a test must not leave a
daemon running, nor reach the one serving the real database.
...
The engine keeps the name `tkengine` for now. Its planned name is `melodyd`
(ADR-0220), deferred so an existing install of the Go `melodyd` is not

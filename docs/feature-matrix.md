...
| MPD/local mapping | Implemented | Each connection profile's configured music-root mapping enables explicit Load as local files and safe direct Melody endpoint playback. Update this folder in MPD is a separate explicit server action. Local publication never implicitly updates MPD (ADRs 0112–0113, 0174). |
...
| Track and album ratings | Implemented | Shared 0-10 star scale (ADR-0179): MPD rating stickers, Melody's native rating commands with album ratings, and a local content-identity store with a rating column and Rate menus in both authorities. Tag storage remains a recorded opt-in follow-up. |

...
| --- | --- | --- |
| Unified MPD/local workspace | Implemented | Authority-bound tabs switch transport, sources, outputs, and available commands. Local tagging/filesystem operations never mutate MPD rows in place; mapped selections reach them by explicit materialization into a local tab (ADR-0180). The old standalone MPD shell is removed; no shell/profile migration remains (ADRs 0058, 0071). |
...
| MPD transport and outputs | Implemented | Play/pause/stop/seek/previous/next, volume, playback modes, advertised ReplayGain, and stock additive outputs. Melody output status/exclusive selection and the Trackknife playback endpoint are independently capability-gated. |
...
| MPD/local mapping | Implemented | Each connection profile's configured music-root mapping enables explicit Load as local files, the Edit tags/ReplayGain/Convert context actions that materialize mapped MPD selections into a local tab and open the dialog there, and safe direct Melody endpoint playback (ADR-0180). Update this folder in MPD is a separate explicit server action. Local publication never implicitly updates MPD (ADRs 0112–0113, 0174). |

...
// ADR-0181: client-owned server list tabs. A persistent working list of MPD
// tracks — copied from the queue, committed searches, or stored playlists —
// rendered from metadata snapshots (so it stays legible offline), edited
// purely client-side, and played by explicitly rewriting the server queue.
...
void BenchMainWindow::refreshMpdListTabChrome(MpdListTab& tab) {
    ...
    tabs_->setTabToolTip(
        index, QStringLiteral("Server working list (client-owned snapshot)%1%2")
...
void BenchMainWindow::addCopyToServerListMenu(QMenu* menu, QTableView* source_view) {
    ...
    connect(create, &QAction::triggered, this, [this, source_view] {
        ...
        bool accepted = false;
        const auto name = QInputDialog::getText(this, QStringLiteral("New server list"),
                                                QStringLiteral("List name:"), QLineEdit::Normal,
                                                QStringLiteral("Server list"), &accepted);
        if (!accepted || name.trimmed().isEmpty()) {
            return;
        }
        createServerListTab(name.trimmed(), std::move(tracks));
    });

// SPDX-License-Identifier: GPL-3.0-only

#include "trackknife/core/sha256.hpp"

#include <bit>
#include <cstring>

namespace trackknife::core {
namespace {

constexpr std::array<std::uint32_t, 64> round_constants{
    0x428a2f98U, 0x71374491U, 0xb5c0fbcfU, 0xe9b5dba5U, 0x3956c25bU, 0x59f111f1U, 0x923f82a4U,
    0xab1c5ed5U, 0xd807aa98U, 0x12835b01U, 0x243185beU, 0x550c7dc3U, 0x72be5d74U, 0x80deb1feU,
    0x9bdc06a7U, 0xc19bf174U, 0xe49b69c1U, 0xefbe4786U, 0x0fc19dc6U, 0x240ca1ccU, 0x2de92c6fU,
    0x4a7484aaU, 0x5cb0a9dcU, 0x76f988daU, 0x983e5152U, 0xa831c66dU, 0xb00327c8U, 0xbf597fc7U,
    0xc6e00bf3U, 0xd5a79147U, 0x06ca6351U, 0x14292967U, 0x27b70a85U, 0x2e1b2138U, 0x4d2c6dfcU,
    0x53380d13U, 0x650a7354U, 0x766a0abbU, 0x81c2c92eU, 0x92722c85U, 0xa2bfe8a1U, 0xa81a664bU,
    0xc24b8b70U, 0xc76c51a3U, 0xd192e819U, 0xd6990624U, 0xf40e3585U, 0x106aa070U, 0x19a4c116U,
    0x1e376c08U, 0x2748774cU, 0x34b0bcb5U, 0x391c0cb3U, 0x4ed8aa4aU, 0x5b9cca4fU, 0x682e6ff3U,
    0x748f82eeU, 0x78a5636fU, 0x84c87814U, 0x8cc70208U, 0x90befffaU, 0xa4506cebU, 0xbef9a3f7U,
    0xc67178f2U,
};

void compress(std::array<std::uint32_t, 8>& state, const std::uint8_t* block) {
    std::array<std::uint32_t, 64> schedule{};
    for (std::size_t word = 0; word < 16U; ++word) {
        schedule[word] = (static_cast<std::uint32_t>(block[word * 4U]) << 24U) |
                         (static_cast<std::uint32_t>(block[word * 4U + 1U]) << 16U) |
                         (static_cast<std::uint32_t>(block[word * 4U + 2U]) << 8U) |
                         static_cast<std::uint32_t>(block[word * 4U + 3U]);
    }
    for (std::size_t word = 16U; word < 64U; ++word) {
        const auto sigma0 = std::rotr(schedule[word - 15U], 7) ^
                            std::rotr(schedule[word - 15U], 18) ^ (schedule[word - 15U] >> 3U);
        const auto sigma1 = std::rotr(schedule[word - 2U], 17) ^
                            std::rotr(schedule[word - 2U], 19) ^ (schedule[word - 2U] >> 10U);
        schedule[word] = schedule[word - 16U] + sigma0 + schedule[word - 7U] + sigma1;
    }
    auto [a, b, c, d, e, f, g, h] = state;
    for (std::size_t round = 0; round < 64U; ++round) {
        const auto big_sigma1 = std::rotr(e, 6) ^ std::rotr(e, 11) ^ std::rotr(e, 25);
        const auto choose = (e & f) ^ (~e & g);
        const auto temporary1 = h + big_sigma1 + choose + round_constants[round] + schedule[round];
        const auto big_sigma0 = std::rotr(a, 2) ^ std::rotr(a, 13) ^ std::rotr(a, 22);
        const auto majority = (a & b) ^ (a & c) ^ (b & c);
        const auto temporary2 = big_sigma0 + majority;
        h = g;
        g = f;
        f = e;
        e = d + temporary1;
        d = c;
        c = b;
        b = a;
        a = temporary1 + temporary2;
    }
    state[0] += a;
    state[1] += b;
    state[2] += c;
    state[3] += d;
    state[4] += e;
    state[5] += f;
    state[6] += g;
    state[7] += h;
}

} // namespace

std::array<std::uint8_t, 32> sha256(const std::string_view bytes) {
    std::array<std::uint32_t, 8> state{0x6a09e667U, 0xbb67ae85U, 0x3c6ef372U, 0xa54ff53aU,
                                       0x510e527fU, 0x9b05688cU, 0x1f83d9abU, 0x5be0cd19U};
    const auto* data = reinterpret_cast<const std::uint8_t*>(bytes.data());
    std::size_t remaining = bytes.size();
    while (remaining >= 64U) {
        compress(state, data);
        data += 64U;
        remaining -= 64U;
    }

    std::array<std::uint8_t, 128> tail{};
    std::memcpy(tail.data(), data, remaining);
    tail[remaining] = 0x80U;
    const std::size_t tail_blocks = remaining + 9U <= 64U ? 1U : 2U;
    const std::uint64_t bit_length = static_cast<std::uint64_t>(bytes.size()) * 8U;
    for (std::size_t byte = 0; byte < 8U; ++byte) {
        tail[tail_blocks * 64U - 1U - byte] = static_cast<std::uint8_t>(bit_length >> (byte * 8U));
    }
    compress(state, tail.data());
    if (tail_blocks == 2U) {
        compress(state, tail.data() + 64U);
    }

    std::array<std::uint8_t, 32> digest{};
    for (std::size_t word = 0; word < state.size(); ++word) {
        digest[word * 4U] = static_cast<std::uint8_t>(state[word] >> 24U);
        digest[word * 4U + 1U] = static_cast<std::uint8_t>(state[word] >> 16U);
        digest[word * 4U + 2U] = static_cast<std::uint8_t>(state[word] >> 8U);
        digest[word * 4U + 3U] = static_cast<std::uint8_t>(state[word]);
    }
    return digest;
}

std::string sha256_hex(const std::string_view bytes) {
    constexpr std::string_view alphabet = "0123456789abcdef";
    const auto digest = sha256(bytes);
    std::string hex;
    hex.reserve(digest.size() * 2U);
    for (const auto byte : digest) {
        hex.push_back(alphabet[byte >> 4U]);
        hex.push_back(alphabet[byte & 0x0FU]);
    }
    return hex;
}

} // namespace trackknife::core
